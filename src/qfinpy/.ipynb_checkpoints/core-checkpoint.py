...
def rate_to_continuous(r, t=1, compounded=1):
    ...
    out = r/t if compounded==0 else (1/compounded) * np.log(1.0 + r*compounded) / t
    if out.size==1:
        return out.item()
    return out
...
def black_scholes_value(type, strike, t, u_price, vol, rf_rate, u_yield=0):
    ...
    t = np.array(t)
    u_price = np.array(u_price)
    vol = np.array(vol)
    rf_rate = np.array(rf_rate)
    u_yield = np.array(u_yield)
    if type=='C':
        den1 = vol * t**0.5
        d1 = (np.log(u_price/strike) + \
...
def trimmed_mean(series, trim=None, axis=-1):
    series = np.asarray(series)
    series.sort(axis=axis)
    if (trim is not None) and (trim<0 or trim>0.5):
        raise RuntimeError("0<= trim <= 0.5")
    if trim is None or trim==0:
        return np.mean(series, axis=axis)
    if trim==0.5:
        return np.median(series, axis=axis)
    n = series.shape[axis]
    k = round(n*trim)
    index_or_slice = slice(k, n-k)
    if axis < 0:
        axis += series.ndim
    slices = tuple(index_or_slice if i == axis else slice(None) for i in range(series.ndim))
    trimmed_series = series[slices]
    if trimmed_series.size==0:
        return np.median(series, axis=axis)
    return np.mean(trimmed_series, axis=axis)

def mad(series, calib=1.4826, axis=-1):
    return np.median(np.abs(series - np.median(series, axis=axis)), axis=axis)

def pct_change(series, axis=-1):
    series = np.asarray(series)
    if axis < 0:
        axis += series.ndim
    slice1 = slice(1, series.shape[axis])
    slice2 = slice(0, series.shape[axis]-1)
    slices1 = tuple(slice1 if i == axis else slice(None) for i in range(series.ndim))
    slices2 = tuple(slice2 if i == axis else slice(None) for i in range(series.ndim))
    return (series[slices1] - series[slices2]) / series[slices2]

def log_change(series, axis=-1):
    series = np.asarray(series)
    if axis < 0:
        axis += series.ndim
    slice1 = slice(1, series.shape[axis])
    ...
    slices1 = tuple(slice1 if i == axis else slice(None) for i in range(series.ndim))
    slices2 = tuple(slice2 if i == axis else slice(None) for i in range(series.ndim))
    return np.log(series[slices1] / series[slices2])
...
def students_t(*n, df, mu=None, sigma=None, bs=None, dtype=np.float64):
    if df<=0:
        raise RuntimeError("df > 0")
    if mu is not None and df<=1:
        raise RuntimeError("mu is undefined for df <= 1")
    if sigma is not None and df<=2:
        raise RuntimeError("sigma does not exist for df <= 2")
    if mu is None:
        mu = np.array([0.0])
    mu = np.asarray(mu, dtype=dtype)
    if sigma is None:
    ...
    if n:
        ...
        if sigma.size>1 and sigma.size!=n[0]:
            raise RuntimeError("if n is specified, size of sigma should be an 1 or n")
        out = np.broadcast_to(mu, n) + np.random.standard_t(df, size=bs+n) * np.broadcast_to(sigma, n)
    else:
        mu, sigma = np.broadcast_arrays(mu, sigma)
        out = mu + np.random.standard_t(df, size=bs+mu.shape) * sigma
    if out.ndim==1 and out.size==1:
        return out.item()
    return out
